...
def node_map(substrate, virtual, req_no):
    ...
    sorder = sorted([a for a in range(substrate.nodes)], key = lambda x: substrate.node_weights[x], reverse=True) # deceing  order
    ...
    for vnode in vorder:
        for snode in sorder:
            if substrate.node_weights[snode] >= virtual.node_weights[vnode] and snode not in assigned_nodes:
                ...
                # print("SN Used Nodes are")
                # print(assigned_nodes.add(snode))
                break
            if snode == sorder[-1]:
                return None
    return map
...
def main():
    ...
    if utilized_nodes!=0:
        ...
        # print("KK User Servers list")
        # print(assigned_nodes)
        # for x in assigned_nodes:
        #     print(x)
    if (accepted != 0):
    ...
    output_dict = {
        ...
        "avg_link": ((utilized_links/len(substrate.edge_weights)))*100,  # Early
